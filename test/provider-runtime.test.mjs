/**
 * AllStakProvider runtime tests using react-test-renderer.
 *
 * Verifies:
 ...
 *   - captured exception is shipped with source=AllStakProvider.ErrorBoundary
 *   - destroyOnUnmount=false leaves the SDK alive across unmount
 *   - destroyOnUnmount=true tears it down
 *   - remount-while-alive reuses the same SDK instance
 *
 * Also exercises the SDK's `flush()` / passthrough behavior with a mock
 * fetch to confirm the wire payloads hit the right endpoints.
 */
...
test('functional fallback receives { error, resetError } and resetError() recovers', () => {
  ResetState();
...
test('remount with default settings reuses the same SDK instance — no double init', () => {
  ...
  assert.equal(secondInstance, firstInstance);
...
test('debug:true logs Initialized line + Navigation auto-instrumentation enabled line', () => {
  ResetState();
  const lines = [];
  ...
  try {
    ...
    act(() => {
      tree = TestRenderer.create(
        React.createElement(
          AllStakProvider,
          { apiKey: 'ask_test_10', debug: true },
          React.createElement('div', null, 'a'),
        ),
      );
    });
    const initLines = lines.filter((l) => l.startsWith('[AllStak] Initialized'));
    const navLines = lines.filter((l) => l.includes('Navigation auto-instrumentation'));
    assert.equal(initLines.length, 1);
    assert.equal(navLines.length, 1);
    assert.match(navLines[0], /enabled|not applied/);
    act(() => { tree.unmount(); });
  } finally {
    console.log = orig;
  }
